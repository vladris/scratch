...
runCmd fuel state ShowState = do printLn state
                                 ok () state
runCmd fuel state (Message str) = do printLn str
                                     ok () state
...
runCmd fuel st (cmd >>= next) = do OK cmdRes newSt <- runCmd fuel st cmd | OutOfFuel => pure OutOfFuel
...
run (More fuel) st (cmd >>= next) = do OK cmdRes newSt <- runCmd fuel st cmd | OutOfFuel => pure OutOfFuel
